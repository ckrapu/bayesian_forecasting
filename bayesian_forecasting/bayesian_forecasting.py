...
"""An FFBS_sample object is used as a sampling step in a PyMC3 model in order to draw
samples of the DLM state vector conditional on emissions/observations and the various
DLM structures F,G,W,V."""

    def __init__(self,vars,F,G,r,n,observationsVarName='observations',check_cov_pd = False):
        ...
        # The attribute 'vars' must be set for PyMC3 to handle this step object appropriately.
        self.vars = vars
        
        # This is the string name given to the variable representing the emissions in the DLM.
        # Because we might call this different things in the larger PyMC3 model, it's handy
...
class FFBS(object):
    def __init__(self,F,G,V,Y,m0,C0,nancheck = True,check_cov_pd=False,evolution_discount=True,deltas = [0.95],W=None):
        ...
        if not evolution_discount:
            ...
            # discount approach.
            try:
                np.linalg.cholesky(self.G) # Fails if G is singular.
            except np.linalg.LinAlgError:
                print 'A discount factor was specified but G is not singular. Retrospective analysis will not be reliable.'
            ...
            if len(deltas) == 1:
                self.discount_matrix = np.identity(self.n) * deltas[0]
        ...
        # All of these need to be defined for the standard DLM (no discounting, known variance)
        # to work.
        self.F    = F    # Dynamic regression vectors 
        self.G    = G    # Static evolution matrix
        
        self.V    = V    # Static observation variance
        self.Y    = Y    # Observations
        self.m0   = m0   # Prior mean on state vector
        self.C0   = C0   # Prior covariance on state vector


        self.is_filtered          = False
        self.is_backward_smoothed = False
        self.nancheck             = nancheck # Determines whether we always check to see whether NaNs have appeared
    ...
    def forward_filter(self):
        ...
        for t in range(T):
            ...
            if self.evolution_discount:
                self.R[t]   = G.dot(self.prior_covariance).dot(G.T).dot(self.discount_matrix)
            else:
                self.R[t]   = G.dot(self.prior_covariance).dot(G.T) + self.W
            ...
            if r == 1:
                self.A[t] = (self.R[t].dot(F[t])/np.squeeze(self.Q[t]))[:,np.newaxis]
    ...
    def backward_smooth(self):
        ...
        if self.evolution_discount:
            ...
            for t in range( self.T-1,-1,-1):
                self.a_r[t] =  self.m[t] if t==( self.T-1) else  (1.0 - discount_matrix).dot(self.m[t]) + discount_matrix.dot(G_inv).dot(self.a_r[t+1])
                self.R_r[t] =  self.C[t] if t==( self.T-1) else  (1.0 - discount_matrix).dot(C[t])+ np.linalg.multi_dot([self.discount_matrix,self.discount_matrix,G_inv,self.R[t+1],G_T_inv])
